...
if current_file is not None:
    ...
    img_link +="eab16d9e-5d32-4f37-b83c-bf0ec2a0eab9/step1.png?format=1000w"
    st.image(img_link,
             width=700,
             caption = "Figure 1: Select the header row and number of skip rows.")
    ...
    ## take away sidebar time delta and units, and put below after calculations
    ## have been made for estimated time delta and units

    
    try:
        ## Use the data from above to assign date/glucose columns
        img_link = "https://images.squarespace-cdn.com/content/v1/5be5c21e75f9ee21b5817cc2/"
        img_link +="241afa5d-e986-46d2-8945-b335d93a2181/step2.png?format=1000w"
        st.image(img_link,
                width=700,
                caption = "Figure 2: Select the Date-Time and Glucose columns.")
    ...
    if date_col is not None and glucose_col is not None:
        try:## try to use the standard pandas.to_datetime without date_format
            ...
            units = 'mg' if mgv > 30 else 'mmol'
            time_delta_ = min([1,5,15], key = lambda x: abs(x-median_time_delta))
