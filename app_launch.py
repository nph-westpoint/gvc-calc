...
if select == options[0]:
    ...
    #st.markdown(f"""<span style="color: #E35731;">{body}</span>""",unsafe_allow_html=True)
    st.markdown(body)
    ...
    body+= "available from the GitHub repository: [GVC-Calc Github](https://github.com/nph-westpoint/gvc-calc). "
    ...
    body+= "1) datetime format (sequentially checks ISO 8601 datetime formats) \n "
    body+= "2) datetime column name \n 3) glucose column name \n "
    body+= "4) time delta between measurements of the device (1 min, 5 mins, or 15 mins) \n "
    body+= "5) glucose level unit of measurement (mg/dL or mmol/L) \n "
    body+= "6) header row location (0 corresponds to the first row) \n "
    body+= "7) number of rows to skip . \n\n "
    ...
    img_link +="df536bb8-4017-442c-94b3-78ca36647fc7/csv_structure.png?format=1000w"
    st.image(img_link,
             width=700,
    ...
    st.markdown(body)
    img_link = "https://images.squarespace-cdn.com/content/v1/5be5c21e75f9ee21b5817cc2/"
    img_link+="819f66be-6566-4bf3-83fb-575125c26c22/workflow.png?format=1000w"
    st.image(img_link,
             width = 700,
...
if select == options[1]:
    ...
    link = "[GVC-Documentation]"
    link += '(https://drive.google.com/file/d/1S8pm9PRVFvEfIUYKr1r2n9qPlu17ny-T/view?usp=drive_link)'
    st.markdown(link)
if select == options[2]:
