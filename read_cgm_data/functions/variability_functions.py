...
def auc_thresh_rate(x, **kwargs):
    """
    auc calculated if both x1,x2 are above threshold, otherwise
        sums 0 for that interval (although we could do better).
        auc_thresh / total days
    """
...
def conga(x,**kwargs):
    ...
    if type_ == 'rGV':
        k = len(delta)
        d_star = np.abs(delta).mean()
        delta = delta.to_frame()
        delta['glucose']=data
        delta['diff2']=(delta['glucose']-d_star)**2
        ## make sure that this is what we want to do - talk to Olawsky if possible.
        congah = np.sqrt(delta['diff2'].sum()/(k-1))
        return congah
    return congah
...
def lability_index(x,**kwargs):
    ...
    try: #if there are too few data values for the data given
        if type_ == 'paper': #Paper assumes mmol/L
            hours = total_hours(Dt,**kwargs)
            weeks = hours/(24*7)
            if units == 'mg':
                Dt = Dt/18.018
            tt = k
            li = ((Dt**2).sum()/tt)/weeks
        if type_ == 'rGV':
            tt = len(Dt)*(k/60)
            li = (Dt**2).sum()/(tt)
    except:
