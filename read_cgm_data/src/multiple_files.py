...
class multiple_CGM(object):
    ...
    def __init__(self,names,
        ...
        self.df = df.copy()
        self.stats_dict = dict(units = self.df)
    ...
    def create_stats_dataframe(self,units='mg'):
        """
        designed to not have to recalculate anything that has already been
            calculated. 
        """
        if units in self.stats_dict.keys():
            return self.stats_dict[units]
        else:
            self.units = units            
        names = self.names
        ...
        self.stats_dict[units] = stats_df.copy()
        return stats_df
